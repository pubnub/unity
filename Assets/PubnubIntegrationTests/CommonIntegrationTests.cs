...
*/
        ...
        public IEnumerator DoSubscribeThenPublishAndParse (bool ssl, string testName, bool asObject, bool withCipher, object message, string expectedStringResponse, bool matchExpectedStringResponse)
        {
            ...
            if (this.Response == null) {
                Assert.Fail(string.Format ("{0}: Null response", testName)); 
            } else {
                bool passed = false;
                if (asObject) {
                ...
                if (!passed) {
                    Assert.Fail(string.Format ("{0}: Not found in {1}", testName, this.Response.ToString ())); 
                }
            }
            ...
            if (this.Response == null) {
                ...
                if (isParamsTest) {
                    ...
                    if (passed) {
                        ...
                        if (this.Response == null) {
                            Assert.Fail(string.Format ("{0}: Null response 2", testName)); 
                        } else {
                            passed = ParseDetailedHistoryResponse (messages, testName, asObject, numberOfMessages / 2, numberOfMessages);
                        }
                    } else {
                        Assert.Fail(string.Format ("{0}: failed one", testName)); 
                    }
                } else {
        ...
        public bool ParseFields (IList<object> fields, object[] messages, string testName, int messageStart, int messageEnd)
        {
            ...
            if ((enumerable != null) && (enumerable.Count > 0)) {
                ...
                //Assert.Fail(string.Format("{0}: {1}", testName, this.Response.ToString ())); 
                return false;
            }
        }
        ...
        public IEnumerator DoConnectedTest (bool ssl, string testName, bool asObject, bool isPresence)
        {
            ...
            if (this.Response == null) {
                ...
                if (!found) {
                    ...
                    Assert.Fail(string.Format ("{0}: Channel not found", testName)); 
                }
            }
            if (isPresence) {
        ...
        void CcCoroutineComplete<T> (object sender, EventArgs ea)
        {
            ...
            if (cea != null && cea.PubnubRequestState != null){
                ...
                Assert.IsTrue(Helpers.GetNamesFromChannelEntities(cea.PubnubRequestState.ChannelEntities, false).Equals (ExpectedChannels)
                    ...
                    Assert.Fail("Cea value not matching");
                }*/
            } else {
                Assert.Fail(string.Format("cea {0}, cea req state ?", (cea==null)?"null":"not null"));
            }
        }
        ...
        void CcCoroutineCompleteError<T> (object sender, EventArgs ea)
        {
            ...
            if (cea != null && cea.PubnubRequestState != null){
                ...
                Assert.IsTrue(Helpers.GetNamesFromChannelEntities(cea.PubnubRequestState.ChannelEntities, false).Equals (ExpectedChannels)
                    ...
                    Assert.Fail("Cea value not matching" + cea.Message.Contains(ExpectedMessage));
                }*/
            } else {
                Assert.Fail(string.Format("cea {0}, cea req state ?", (cea==null)?"null":"not null"));
            }
        ...
        public void TestBounce<T>(string url, int timeout, int pause, string[] channels, bool resumeOnReconnect, 
            ...
            if (failTest) {
                ...
                Assert.Fail("www not null and done");
            }
        }
