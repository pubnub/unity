...
namespace PubNubMessaging.Tests
{
	...
	public class CommonIntergrationTests
	{
			...
			if (this.Response == null) {
				...
				if (asObject) {
					...
					if (matchExpectedStringResponse) {
						...
						UnityEngine.Debug.Log (string.Format ("{0}: !matchExpectedStringResponse", DateTime.Now.ToString (), this.Response, expectedStringResponse));
		...
		public IEnumerator DoPresenceSubscribeAndParse (bool ssl, string testName, bool asObject)
		{
			string channel = Init (testName, ssl);

			UnityEngine.Debug.Log (string.Format ("{0} {1}: Start coroutine ", DateTime.Now.ToString (), testName));
			if (asObject) {
				pubnub.Presence<object> (channel, this.DisplayReturnMessage, this.DisplayReturnMessageDummy, this.DisplayErrorMessage);
			} else {
				pubnub.Presence<string> (channel, this.DisplayReturnMessage, this.DisplayReturnMessageDummy, this.DisplayErrorMessage);
			}
			yield return new WaitForSeconds (CommonIntergrationTests.WaitTimeBetweenCalls); 
			CommonIntergrationTests commonSubscribe = new CommonIntergrationTests ();
			commonSubscribe.DeliveryStatus = false;
			...
			UnityEngine.Debug.Log (string.Format ("{0} {1}: Running Subscribe ", DateTime.Now.ToString (), testName));
			//try {
			pubnub.Subscribe<string> (channel, commonSubscribe.DisplayReturnMessageDummy, commonSubscribe.DisplayReturnMessageDummy, commonSubscribe.DisplayErrorMessage);
			//} catch (Exception ex) {
			//	UnityEngine.Debug.Log (string.Format ("{0} {1}: exception ", ex.ToString (), testName));
			//}
			UnityEngine.Debug.Log (string.Format ("{0} {1}: Waiting ", DateTime.Now.ToString (), testName));
